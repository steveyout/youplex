...
function cacheSet(key, value, ttlMs = CACHE_TTL_MS) {
  ...
  // Basic loop-vacuum to keep the map small
  if (cache.size > 500) {
    const now = Date.now();
 ...
 *   Optional: force a single provider by id (e.g. 'videasy').
 *   Default: run all enabled providers in parallel and return the first
 *   match — far faster than sequential fallback, which let one slow provider
 *   push the whole scrape past the gateway timeout.
...
export async function getSources(request, options = {}) {
  ...
  /**
   * Run every candidate in parallel, each bounded by PROVIDER_TIMEOUT_MS.
   * The first successful extraction resolves the race; if none succeed we
   * wait for all to settle (≤ timeout) and report the highest-priority error.
   */
  const outcome = await new Promise((resolve) => {
    let remaining = candidates.length;
    /** @type {Map<number, import('./types').ExtractionResult>} */
    ...
    const settle = () => {
      if (failures.size === 0) {
        resolve({
          ...
          sources: [],
          subtitles: [],
          provider: candidates[0]?.name ?? 'none',
          error: 'All extraction providers failed or timed out',
        });
        return;
      }
      const best = [...failures.entries()].sort((a, b) => a[0] - b[0])[0][1];
      resolve({
        success: false,
        sources: [],
        subtitles: [],
        provider: best.provider,
        error: best.error || 'All extraction providers failed',
      });
    };

    for (const provider of candidates) {
      const key = cacheKey(request, provider.name);
      withTimeout(provider.extract(request), provider.name).then((result) => {
        if (result.success && result.sources.length > 0) {
          if (useCache) cacheSet(key, result);
          resolve(result);
          return;
        }
        failures.set(provider.priority, result);
        remaining -= 1;
